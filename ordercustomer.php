...
//                print '<pre>'.print_r("i : ".$i,1).'</pre>';
//                print '<pre>'.print_r("check",1).'</pre>';
//                print '<pre>'.print_r(GETPOST('check' . $i, 'alpha'),1).'</pre>';
//                print '<pre>'.print_r("fourn",1).'</pre>';
//                print '<pre>'.print_r(GETPOST('fourn' . $i, 'int'),1).'</pre>';
//                print '<pre>'.print_r("fourn_free",1).'</pre>';
//                print '<pre>'.print_r(GETPOST('fourn_free' . $i, 'int'),1).'</pre>';
...
//            exit;
            ...
            foreach ($suppliers as $idsupplier => $supplier) {
                ...
                $order_id = $order->id;
                if (!empty($order_id) && $action == 'valid-propal') {
                    $getNomUrlConcat .= ' ' . $order->getNomUrl();
                }
                ...
                foreach ($supplier['lines'] as $line) {
                    ...
                    foreach ($order->lines as $lineOrderFetched) {
                        ...
                        if (!empty($id_line_element_element)) {
                            ...
                            if ($order->element == 'order_supplier') {
                                $order->updateline($lineOrderFetched->id, $lineOrderFetched->desc, // FIXME: The current existing line may very well not be at the same purchase price
                                    $lineOrderFetched->pu_ht,

                                    $lineOrderFetched->qty + $line->qty, $remise_percent, $lineOrderFetched->tva_tx);
                            } else {
                                if ($order->element == 'supplier_proposal') {
                                    $order->updateline($lineOrderFetched->id, $prodfourn->fourn_unitprice, //$lineOrderFetched->pu_ht is empty,
                                        $lineOrderFetched->qty + $line->qty, $remise_percent, $lineOrderFetched->tva_tx, 0, //$txlocaltax1=0,
                                        0, //$txlocaltax2=0,
                    ...
                    if (!$done) {
//                        print '<pre>'.print_r($line,1).'</pre>';
                        if ($order->element == 'order_supplier') {
                            $cf_line_id = $order->addline($line->desc, $line->subprice, $line->qty, $line->tva_tx, null, null, $line->fk_product, // We need to pass fk_prod_fourn_price to get the right price.
                                $line->fk_prod_fourn_price, $line->ref_fourn, $line->remise_percent, 'HT', 0, $line->product_type, $line->info_bits, false // $notrigger
                                , null // $date_start
                                , null // $date_end
                                , $line->array_options, null, 0, $line->origin, $line->origin_id);
//                            print '<pre>'.print_r($cf_line_id,1).'</pre>';
//                            print '<pre>'.print_r($order->errors,1).'</pre>';
//                            print '<pre>'.print_r($order->error,1).'</pre>';
//                            print '<pre>'.print_r($db->lastquery(),1).'</pre>';
//                            exit;
                            ...
                            if ($order->element == 'supplier_proposal') {
                                $order->addline($line->desc, $line->subprice, $line->qty, $line->tva_tx, null, null, $line->fk_product, $line->remise_percent, 'HT', 0, //$pu_ttc=0,
            ...
            if ($action == 'valid-order') {
                ...
                if ($action == 'valid-propal' && !empty($getNomUrlConcat)) {
                    setEventMessage($langs->trans('SupplierProposalSuccessfullyCreated') . $getNomUrlConcat, 'mesgs');
                }
            }
        }
...
if ($resql || $resql2) {
    ...
    if ($sref || $snom || $sall || $salert || GETPOST('search', 'alpha')) {
        ...
        if (!getDolGlobalInt('SOFO_USE_DELIVERY_TIME')) {
            print_barre_liste($title, $page, 'ordercustomer.php', $filters, $sortfield, $sortorder, '', $num);
        }
    } else {
        $filters = '&sref=' . $sref . '&snom=' . $snom;
    ...
    print'</div>';
    print '<form action="' . $_SERVER['PHP_SELF'] . '?id=' . GETPOST('id', 'int') . '&projectid=' . (!empty($_REQUEST['projectid']) ? $_REQUEST['projectid'] : '') . $yesno . '" method="post" name="formulaire">' . '<input type="hidden" name="id" value="' . GETPOST('id', 'int') . '">' . '<input type="hidden" name="token" value="' . $_SESSION['newtoken'] . '">' . '<input type="hidden" name="sortfield" value="' . $sortfield . '">' . '<input type="hidden" name="sortorder" value="' . $sortorder . '">' . '<input type="hidden" name="type" value="' . $type . '">' . '<input type="hidden" name="linecount" value="' . ($num + $num2) . '">' . '<input type="hidden" name="group_lines_by_product" value="' . $group_lines_by_product . '">' . '<input type="hidden" name="fk_commande" value="' . GETPOST('fk_commande', 'int') . '">' . '<input type="hidden" name="show_stock_no_need" value="' . GETPOST('show_stock_no_need', 'none') . '">';
    ...
    print '<div style="text-align:right">	  </div>' . '<table class="liste" width="100%">';
    ...
    if (getDolGlobalString('SOFO_USE_DELIVERY_TIME')) {
        ...
        print '<tr class="liste_titre">' . '<td colspan="' . $colspan . '">' . $langs->trans('NbWeekToReplenish') . '<input type="text" name="week_to_replenish" value="' . $week_to_replenish . '" size="2"> ' . '<input type="submit" value="' . $langs->trans('ReCalculate') . '" /></td>';
    ...
    if (!GETPOSTISSET('button_removefilter_x') && GETPOSTISSET('finished', 'none')) {
        $param .= '&finished=' . GETPOST('finished', 'none');
    }

    // Lignes des titres
    print '<tr class="liste_titre_filter">' . '<th class="liste_titre"><input type="checkbox" onClick="toggle(this)" /></th>';
    print_liste_field_titre($langs->trans('Ref'), 'ordercustomer.php', 'prod.ref', $param, 'id=' . GETPOST('id', 'int'), '', $sortfield, $sortorder);
    print_liste_field_titre($langs->trans('Label'), 'ordercustomer.php', 'prod.label', $param, 'id=' . GETPOST('id', 'int'), '', $sortfield, $sortorder);
    print_liste_field_titre($langs->trans('Nature'), 'ordercustomer.php', 'prod.finished', $param, 'id=' . GETPOST('id', 'int'), '', $sortfield, $sortorder);
    if (!empty($conf->categorie->enabled) && getDolGlobalString('SOFO_DISPLAY_CAT_COLUMN')) {
        print_liste_field_titre($langs->trans("Categories"), 'ordercustomer.php', 'cp.fk_categorie', $param, 'id=' . GETPOST('id', 'int'), '', $sortfield, $sortorder);
    }
    if (!empty($conf->service->enabled) && $type == 1) {
        print_liste_field_titre($langs->trans('Duration'), 'ordercustomer.php', 'prod.duration', $param, 'id=' . GETPOST('id', 'int'), 'align="center"', $sortfield, $sortorder);
    }

    if ($dolibarr_version35) {
        print_liste_field_titre($langs->trans('DesiredStock'), 'ordercustomer.php', 'prod.desiredstock', $param, 'id=' . GETPOST('id', 'int'), 'align="right"', $sortfield, $sortorder);
    }
    ...
    print_liste_field_titre($stocklabel, 'ordercustomer.php', 'stock_physique', $param, 'id=' . GETPOST('id', 'int'), 'align="right"', $sortfield, $sortorder);

    if (!empty($conf->of->enabled) && getDolGlobalString('OF_USE_DESTOCKAGE_PARTIEL')) {
        dol_include_once('/of/lib/of.lib.php');
        print_liste_field_titre('Stock théo - OF', 'ordercustomer.php', 'stock_theo_of', $param, 'id=' . GETPOST('id', 'int'), 'align="right"', $sortfield, $sortorder);
    }

    print_liste_field_titre($langs->trans('Diff'), 'ordercustomer.php', '', $param, '', 'align="right"', $sortfield, $sortorder);

    print_liste_field_titre($langs->trans('Ordered'), 'ordercustomer.php', '', $param, 'id=' . GETPOST('id', 'int'), 'align="right"', $sortfield, $sortorder);
    print_liste_field_titre($langs->trans('StockToBuy'), 'ordercustomer.php', '', $param, 'id=' . GETPOST('id', 'int'), 'align="right"', $sortfield, $sortorder);

    if (getDolGlobalString('FOURN_PRODUCT_AVAILABILITY')) {
        print_liste_field_titre($langs->trans("Availability"));
    }

    print_liste_field_titre($langs->trans('Supplier'), 'ordercustomer.php', '', $param, 'id=' . GETPOST('id', 'int'), 'align="right"', $sortfield, $sortorder);

    print '<th class="liste_titre" >&nbsp;</th>';

    print '</tr>' . // Lignes des champs de filtre
        '<tr class="liste_titre_filter">' . '<td class="liste_titre">&nbsp;</td>' . '<td class="liste_titre">' . '<input class="flat" type="text" name="sref" value="' . $sref . '">' . '</td>' . '<td class="liste_titre">' . '<input class="flat" type="text" name="snom" value="' . $snom . '">';
    '</td>';
        ...
        foreach ($TProducts as $key => $objp) {
            if (empty($objp->level)) {
                if (array_key_exists($objp->rowid, $TProductQtyChecked)) {
                    ...
                    $TProductQtyChecked[$objp->rowid] = $objp;
                }
            } else {
                $TProductSubLevel[] = $objp;
            }
        }
    ...
    foreach ($TProducts as $objp) {
        ...
        if (!empty($group_lines_by_product) && empty($objp->level)) {
            if (in_array($objp->rowid, $TProductIDAlreadyChecked)) {
                continue;
            } else {
                $TProductIDAlreadyChecked[$objp->rowid] = $objp->rowid;
            }
        }
        ...
        if (getDolGlobalString('SOFO_DISPLAY_SERVICES') || $objp->fk_product_type == 0) {
            ...
            if (!$justOFforNeededProduct) {
                if ($week_to_replenish > 0) {
                    ...
                    if (getDolGlobalString('USE_VIRTUAL_STOCK') || getDolGlobalString('SOFO_USE_VIRTUAL_ORDER_STOCK')) {
                        //compute virtual stockshow_stock_no_need
                        $prod->fetch($prod->id);
                        if ((!getDolGlobalString('STOCK_CALCULATE_ON_VALIDATE_ORDER') || getDolGlobalString('SOFO_USE_VIRTUAL_ORDER_STOCK')) && !getDolGlobalString('SOFO_DO_NOT_USE_CUSTOMER_ORDER')) {
                            ...
                            if (!empty($objp->level)) {
                                $stock_commande_client = $stock_commande_client + $objp->qty;
                            }
                        } else {
                            $stock_commande_client = 0;
                        }
                        ...
                        if (!empty($conf->expedition->enabled) && (getDolGlobalString('STOCK_CALCULATE_ON_SHIPMENT') || getDolGlobalString('STOCK_CALCULATE_ON_SHIPMENT_CLOSE'))) {
            ...
            $warning = '';
            if (!empty($objp->seuil_stock_alerte) && ($stock < $objp->seuil_stock_alerte)) {
            ...
            if ((empty($prod->type) && $stocktobuy == 0 && GETPOST('show_stock_no_need', 'none') != 'yes') || ($prod->type == 1 && $stocktobuy == 0 && GETPOST('show_stock_no_need', 'none') != 'yes' && getDolGlobalString('STOCK_SUPPORTS_SERVICES'))) {
                $i++;
                continue;
            }
            ...
            $prod->load_stock();
            $champs .= '<td align="right" >' . $warning . (((getDolGlobalString('STOCK_SUPPORTS_SERVICES') && $prod->type == 1) || empty($prod->type)) ? $prod->stock_theorique : img_picto('', './img/no', '', 1)) . //$stocktobuy
                '</td>';
            if (!empty($conf->of->enabled) && getDolGlobalString('OF_USE_DESTOCKAGE_PARTIEL')) {
                /*					dol_include_once('/of/lib/of.lib.php');
                                    $prod->load_stock();
            ...
            // déjà present
            $champs .= '<td align="right">' . ($objLineNewQty->oldQty ?? '') . '</td>';
            //Commandé
            ...
            $champs .= '</td>' . '<td align="right">' . '<input type="text" name="tobuy' . $i . '" value="' . (!getDolGlobalString('SOFO_QTY_LINES_COMES_FROM_ORIGIN_ORDER_ONLY') ? $stocktobuy : (empty($group_lines_by_product) ? $objp->qty : $objLineNewQty->qty ?? 0)) . '" ' . $disabled . ' size="3"> <span class="stock_details" prod-id="' . $prod->id . '" week-to-replenish="' . $week_to_replenish . '">' . img_help(1, $help_stock) . '</span></td>';
...
//            print "toto";
//            print '<pre>'.print_r($orderLine->array_options,1).'</pre>';
            if ($conf->specialprices->enabled && !empty($orderLine->array_options['options_special_price']) && $orderLine->array_options['options_special_price'] > 0) {
//        print "toto";
                require_once dol_buildpath('/specialprices/class/specialprice.class.php');
                $specialPrice = new SpecialPrice($db);
                $specialPrice->fetch($orderLine->array_options['options_special_price']);
                $soc = new Societe($db);
                $soc->fetch($specialPrice->fk_supplier);
                $champs .= '<td align="left" data-info="special_price" >' . $specialPrice->getNomUrl() . '</td>';
                $champs .= '<input type="hidden" name="specialprice' . $i . '" value="' . $specialPrice->id . '"/>';
                $champs .= '<input type="hidden" name="fourn' . $i . '" value="' . $specialPrice->fk_supplier . '"/>'; 
            } else {
                $champs .= '<td align="right" data-info="fourn-price" >' . TSOFO::select_product_fourn_price($prod->id, 'fourn' . $i, $selectedSupplier, $selectedPrice) . '</td>';
            }
            print $champs;
            ...
            if (empty($TSupplier)) {
                ...
                $TSupplier = array_intersect($prod->list_suppliers(), $TSupplier);
            }
    ...
    if (!empty($resql2)) {
        if (isset($j)) {
            while ($j < min($num2, $limit)) {
                ...
                print '<tr ' . $bc[$var] . '>' . '<td><input type="checkbox" class="check" name="check' . $i . '"' . $disabled . '></td>' . '<td>' . $picto . " " . $objp->description . '</td>' . '<td>' . $objp->description;
    ...
    print '</table>' . '<table width="100%" style="margin-top:15px;">';
    print '<tr>';
    ...
    print '</tr>';
    print '<tr><td>&nbsp;</td></tr>';
    print '<tr><td align="right">' . '<button class="butAction" type="submit" name="action" value="valid-propal">' . $langs->trans("GenerateSupplierPropal") . '</button>' . '<button class="butAction" type="submit" name="action" value="valid-order">' . $langs->trans("GenerateSupplierOrder") . '</button>' . '<button class="butAction" type="submit" name="action" value="view-valid-order">' . $langs->trans("ViewSupplierOrderGenerated") . '</button>' . '</td></tr></table>' . '</form>';
...
function _prepareLine($i, $actionTarget = 'order') {
    ...
    if (!GETPOST('tobuy_free' . $i, 'none')) {
        ...
        $array_options = array();
        //$array_options["options_special_price"] = GETPOST('specialprice' . $i, 'int');
        if (!empty($lineid)) {
        ...
        if(empty($array_options["options_special_price"]) || $array_options["options_special_price"] <= 0 || empty($conf->specialprices->enabled)){
...
//            $obj->subprice
        ...
        if ($obj) {
            ...
            if (empty($line->remise_percent) && !empty($obj->remise_supplier)) {
                $line->remise_percent = $obj->remise_supplier;
            }
...
function _appliCond($order, $commandeClient) {
    ...
    if (getDolGlobalString('SOFO_GET_EXTRAFIELDS_FROM_ORDER')) {
        ...
        if (!empty($commandeClient->array_options) && !empty($extrafieldsOrderSupplier->attributes) && array_key_exists($order->table_element, $extrafieldsOrderSupplier->attributes) && array_key_exists('label', $extrafieldsOrderSupplier->attributes[$order->table_element]) && !empty($extrafieldsOrderSupplier->attributes[$order->table_element]['label'])) {
            foreach ($commandeClient->array_options as $key => $val) {
                $key = str_replace('options_', '', $key);
                if (array_key_exists($key, $extrafieldsOrderSupplier->attributes[$order->table_element]['type']) && $extrafieldsOrderSupplier->attributes[$order->table_element]['type'][$key] == $extrafieldsCustomerOrder->attributes[$commandeClient->table_element]['type'][$key]) {
